...
/*                    else
                    ...
                    answers = getAnswerList(record[4].split(optionSplitter));
                    markCorrectAnswers(answers, record[5]);
                    questions.add(new Question(-1, topic, answers, record[2], record[3]));

                } else {
                    logger.info("Invalid record - need 6 columns in record to load - Record Index " + recordIndex);
                }
                recordIndex++;
            }
