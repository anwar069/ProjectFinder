...
//        this.close();
    ...
    public ArrayList<Question> getQuestionsByTopicID(int topicID) {
        ...
        String[] allColumns = {DatabaseHelper.COLUMN_ID, DatabaseHelper.TOPIC_ID
                , DatabaseHelper.QUESTION_TEXT, DatabaseHelper.ADDITIONAL_INFO};
        ...
        while (!cursor.isAfterLast()) {
            ...
            questions.add(new Question(cursor.getInt(0), topic, answers, cursor.getString(2), cursor.getString(3)));
            cursor.moveToNext();
        }
        cursor.close();
        //this.close();
